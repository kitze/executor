...
it("keeps the first concurrent in-process browser decision terminal until settlement", async () => {
  ...
  const first = {
    response: { action: "decline" as const, content: { reason: "first" } },
    orgWriteAccess: "denied" as const,
  };
  const later = {
    response: { action: "accept" as const, content: { reason: "later" } },
...
it("retains the exact live response object and first approver's role", async () => {
  const approvals = makeInProcessBrowserApprovalStore();
  const response = { action: "accept" as const };
  const first = { response, orgWriteAccess: "denied" as const };
  await Effect.runPromise(approvals.recordResponse("live", first));
  const replay = await Effect.runPromise(
    approvals.recordResponse("live", {
      response: { action: "accept" },
      orgWriteAccess: "allowed",
    }),
  );
  expect(replay).toBe(first);
  expect(replay.response).toBe(response);
  expect(replay.orgWriteAccess).toBe("denied");
  approvals.forget("live");
  expect(await Effect.runPromise(approvals.store.takeResponse("live"))).toBeNull();
});
