...
/** Run a list/sidebar health probe through an independent browser request.
 * The typed Effect HTTP operation is retained for the detail/manual flow, but
 * parallel uses of that shared operation can cancel one another. A standalone
 * same-origin fetch per row lets every saved connection finish its own probe. */
export async function runConnectionHealthCheck(
  connection: Connection,
  options?: { readonly force?: boolean },
): Promise<HealthCheckResult> {
  const query = options?.force ? {} : revalidateQuery(connection.lastHealth);
  const params = new URLSearchParams();
  if (query.ifStaleMs !== undefined) {
    params.set("ifStaleMs", String(query.ifStaleMs));
  }

  const path =
    `/api/connections/${encodeURIComponent(connection.owner)}` +
    `/${encodeURIComponent(connection.integration)}` +
    `/${encodeURIComponent(connection.name)}/health`;
  ...
  // oxlint-disable-next-line executor/no-try-catch-or-throw, executor/no-error-constructor -- boundary: browser-only fetch helper exposes the native Promise rejection contract consumed by React effects
...
export function useConnectionsHealth(
  ...
  useEffect(() => {
    ...
    for (const connection of connections) {
      ...
      // oxlint-disable-next-line executor/no-promise-catch -- boundary: fire-and-forget React background probe; a persisted verdict remains the intentional fallback on transport failure
      void runConnectionHealthCheck(connection)
        .then((result) => {
          ...
          revalidated.current.set(key, result.checkedAt);
          setLiveProbes((current) => new Map(current).set(key, result));
          if (result.status !== (last?.status ?? "unknown")) {
            invalidateConnections(connection.owner);
          }
        })
        .catch(() => {});
    }
  }, [connections, invalidateConnections, options?.revalidate]);
