    ...
    const execute = (
      ...
      return Effect.gen(function* () {
        ...
        const wrapInvocationError = <A, E>(
          ...
          return effect.pipe(
            Effect.mapError((cause) => {
              // This SDK error has a fixed public message and no payload. Rebuild
              // it so redaction cannot erase its prototype-based userMessage.
              const safeCause = Predicate.isTagged(cause, "OrgWriteDeniedError")
                ? new OrgWriteDeniedError()
                : opaqueValues
                  ? opaqueValues.redact(cause)
                  : cause;
